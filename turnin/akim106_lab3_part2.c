...
int main(void) {
    ...
    while (1) {
      tmpIN = PINA & 0x0F; //Getting right 4 bits of input and resetting output
      tmpOUT = 0x00;

      if (tmpIN > 0x00) { //Checking fuel light 1
        tmpOUT = tmpOUT | 0x01; //Setting right bit to 1 if fuel level 1 or more
      }
      tmpOUT = tmpOUT << 1; //Pushing the right bit leftward

      if (tmpIN > 0x02) { //Checking 2
        tmpOUT = tmpOUT | 0x01;
      }
      tmpOUT = tmpOUT << 1;

      if (tmpIN > 0x04) { //Checking 3
        tmpOUT = tmpOUT | 0x01;
      }
      tmpOUT = tmpOUT << 1;
      
      if (tmpIN > 0x06) { //Checking 4
        tmpOUT = tmpOUT | 0x01;
      }
      tmpOUT = tmpOUT << 1;
      
      if (tmpIN > 0x09) { //Checking 5
        tmpOUT = tmpOUT | 0x01;
      }
      tmpOUT = tmpOUT << 1;
      
      if (tmpIN > 0x0C) { //Checking 6
        tmpOUT = tmpOUT | 0x01;
      }
